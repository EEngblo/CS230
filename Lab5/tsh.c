...
void eval(char *cmdline) {
	...
	if (builtin_cmd_done) return;		  // and return
	...
	if (pid == 0) { // child
		setpgid(0, 0); // set process group to itself
		sigprocmask(SIG_SETMASK, &sig_old, NULL);

		if (execve(argv[0], argv, environ) < 0) { // run the job in the context of the child
			printf("%s: Command not found.\n", argv[0]); // only executed when argv[0] is invalid command
			exit(0);
		}
	} else { // parent
